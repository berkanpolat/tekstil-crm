-- Genel Rapor v2: kanal × huni, kampanya × huni, il × huni, önceki dönem, eğilim, gün×saat.
-- Karar: Tuna, 22 Eyl 2026 (pazarlama gözüyle rapor planı, Faz 1).

insert into public.settings (key, value, category, description)
values ('reports.min_rate_base', '5', 'reports', 'Raporlarda oran gösterimi için en küçük payda (altında "—")')
on conflict (key) do nothing;

-- ── metric_requests: + p_marketing filtresi, + by_dow_hour ─────────────────
drop function if exists public.metric_requests(timestamptz, timestamptz, uuid, bigint, bigint, bigint);
drop function if exists metrics.metric_requests(timestamptz, timestamptz, uuid, bigint, bigint, bigint);
...
create or replace function public.metric_requests(p_from timestamptz, p_to timestamptz, p_scope_user uuid default null, p_channel bigint default null, p_category bigint default null, p_province bigint default null, p_marketing bigint default null)
returns jsonb language sql stable as $$ select metrics.metric_requests(p_from, p_to, p_scope_user, p_channel, p_category, p_province, p_marketing) $$;
grant execute on function public.metric_requests(timestamptz, timestamptz, uuid, bigint, bigint, bigint, bigint) to authenticated;
revoke execute on function public.metric_requests(timestamptz, timestamptz, uuid, bigint, bigint, bigint, bigint) from anon;

-- ── metric_genel v2 ──────────────────────────────────────────────────────
-- Tek base CTE iki dönemi tarar ([v_pf, p_to)); cur = bu dönem. Huni tanımları v1 ile aynı.
-- Kırılımlar: kanal / kampanya (utm_campaign) / il → metrics.huni_satiri ile tek biçim.
drop function if exists public.metric_genel(timestamptz, timestamptz);
drop function if exists metrics.metric_genel(timestamptz, timestamptz);

create or replace function metrics.huni_satiri(
  p_key text, p_label text, p_talep bigint, p_onceki bigint, p_teklif bigint, p_numune bigint, p_siparis bigint,
  p_reddedilen bigint, p_bekleyen bigint, p_gecersiz bigint, p_ilk_yanit numeric, p_sla_met bigint, p_sla_missed bigint,
  p_teklif_yanit numeric
) returns jsonb language sql immutable set search_path to '' as $$
  select jsonb_build_object(
    'key', p_key, 'label', p_label,
    'talep', p_talep, 'onceki_talep', p_onceki, 'degisim_pct', metrics.pct(p_talep, p_onceki),
    'teklif', p_teklif, 'teklif_orani', case when p_talep > 0 then round(100.0 * p_teklif / p_talep, 1) end,
    'numune', p_numune, 'numune_orani', case when p_teklif > 0 then round(100.0 * p_numune / p_teklif, 1) end,
    'siparis', p_siparis, 'siparis_orani', case when p_talep > 0 then round(100.0 * p_siparis / p_talep, 1) end,
    'numune_siparis_orani', case when p_numune > 0 then round(100.0 * p_siparis / p_numune, 1) end,
    'reddedilen', p_reddedilen, 'red_orani', case when p_talep > 0 then round(100.0 * p_reddedilen / p_talep, 1) end,
    'bekleyen', p_bekleyen, 'gecersiz', p_gecersiz,
    'ilk_yanit_saat', round(p_ilk_yanit, 1),
    'sla_orani', case when (p_sla_met + p_sla_missed) > 0 then round(100.0 * p_sla_met / (p_sla_met + p_sla_missed), 1) end,
    'sla_met', p_sla_met, 'sla_missed', p_sla_missed,
    'teklif_yanit_saat', round(p_teklif_yanit, 1))
$$;
...
begin
  ...
  grp as (
    select g.tur, g.key, g.label, g.sira,
      ...
      count(*) filter (where b.cur and b.sla_deadline is not null and b.sla_deadline < now() and (b.first_quote_at is null or b.first_quote_at > b.sla_deadline)) sla_missed,
...
create or replace function public.metric_genel(p_from timestamptz, p_to timestamptz, p_marketing bigint default null)
returns jsonb language sql stable security definer set search_path to ''
as $$ select metrics.metric_genel(p_from, p_to, p_marketing) $$;
grant execute on function public.metric_genel(timestamptz, timestamptz, bigint) to authenticated;
revoke execute on function public.metric_genel(timestamptz, timestamptz, bigint) from anon;
