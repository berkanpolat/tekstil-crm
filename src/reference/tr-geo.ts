...
/** İl adları (dropdown seçenekleri için). */
export const TR_PROVINCE_NAMES: string[] = TR_PROVINCES.map((p) => p.name)

/** Verilen il için ilçeler; il bulunamazsa boş dizi. Case/whitespace toleranslı. */
export function districtsOf(province: string | null | undefined): string[] {
  if (!province) return []
  const key = province.trim().toLocaleLowerCase('tr')
  return TR_PROVINCES.find((p) => p.name.toLocaleLowerCase('tr') === key)?.districts ?? []
}
